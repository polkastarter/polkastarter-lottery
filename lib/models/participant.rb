...
class Participant
  ...
  def initialize(address:, balance:, recent_winner:, nft_rare_holder: false, nft_common_holder: false)
    ...
    @balance = (balance && balance >= 0) ? balance : 0 # guarding agains nil or negative balances (corner cases that can arise 
                                                       # from the fact that sanpshot are taken at slightly differente moments 
